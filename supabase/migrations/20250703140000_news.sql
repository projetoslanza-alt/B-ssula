...
ALTER TABLE news_publication_teams ENABLE ROW LEVEL SECURITY;
ALTER TABLE news_publication_groups ENABLE ROW LEVEL SECURITY;
ALTER TABLE news_attachments ENABLE ROW LEVEL SECURITY;

CREATE POLICY news_publications_select ON news_publications
  FOR SELECT USING (user_can_view_news(news_publications));

CREATE POLICY news_publications_manage ON news_publications
  FOR ALL USING (
    tenant_id = user_active_tenant_id()
    AND is_member_of_tenant(tenant_id)
    AND has_permission('news.manage')
  )
  WITH CHECK (
    tenant_id = user_active_tenant_id()
    AND has_permission('news.manage')
...
CREATE POLICY news_publication_teams_select ON news_publication_teams
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM news_publications np
      WHERE np.id = publication_id AND user_can_view_news(np)
    )
  );

CREATE POLICY news_publication_teams_manage ON news_publication_teams
  FOR ALL USING (tenant_id = user_active_tenant_id() AND has_permission('news.manage'))
  WITH CHECK (tenant_id = user_active_tenant_id() AND has_permission('news.manage'));

CREATE POLICY news_publication_groups_select ON news_publication_groups
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM news_publications np
      WHERE np.id = publication_id AND user_can_view_news(np)
...
CREATE POLICY news_publication_groups_manage ON news_publication_groups
  FOR ALL USING (tenant_id = user_active_tenant_id() AND has_permission('news.manage'))
  WITH CHECK (tenant_id = user_active_tenant_id() AND has_permission('news.manage'));

CREATE POLICY news_attachments_select ON news_attachments
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM news_publications np
      WHERE np.id = publication_id AND user_can_view_news(np)
...
CREATE POLICY news_attachments_manage ON news_attachments
  FOR ALL USING (tenant_id = user_active_tenant_id() AND has_permission('news.manage'))
  WITH CHECK (tenant_id = user_active_tenant_id() AND has_permission('news.manage'));
